...
import {
  ...
  useGetDeals,
  useToggleDealStatus,
} from '@/hooks/api/store/deliveries/product-management/deals';
import { AppButton } from '@/components/shared/AppButton';
import { Heading } from '@/components/shared/Heading';
import AddDealDrawer from '@/components/store/deliveries/product-management/deals/add-deal/AddDealDrawer';
import DealsTable from '@/components/store/deliveries/product-management/deals/table/DealsTable';
import EditDealDrawer from '@/components/store/deliveries/product-management/deals/edit-deal/EditDealDrawer';
import {
  calculateFinalDealPrice,
  toNumberOrNull,
} from '@/components/store/deliveries/product-management/deals/table/dealDrawer.utils';
...
const Deals = () => {
  ...
  const { data, isLoading, isError, error } = useGetDeals(storeId || '', {
    enabled: !!storeId,
  });
  const { mutateAsync: deleteDeal } = useDeleteDeal(storeId || '');
  const {
    mutate: toggleDealStatus,
    isPending: isTogglingDealStatus,
    variables: toggleStatusVariables,
  } = useToggleDealStatus(storeId || '', {
    onSuccess: () => {
  ...
  return (
    <div className="space-y-7">
      ...
      <DealsTable
        deals={deals}
        fetchAll={() => fetchAllReport<RawDeal>(`/apps/deliveries/store/deals/${storeId}`).then((rows) => rows.map(mapDeal))}
