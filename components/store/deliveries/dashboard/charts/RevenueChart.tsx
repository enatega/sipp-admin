...
import { StoreGraphPoint, StoreGraphSeries } from '@/types';
import { Skeleton } from '@/components/ui/skeleton';
import { IStoreRevenueChart } from '../types';
...
function toChartData(series: StoreGraphSeries[], granularity: string) {
  if (!series || series.length === 0 || !series[0].points) {
    return { labels: [], datasets: [] };
  }

  const labels = series[0].points.map((p: StoreGraphPoint) =>
    getLabel(p.t, granularity),
  );

  const datasets: RevenueChartDataset[] = series.map((s: StoreGraphSeries) => {
    const color =
      COLORS[s.key as keyof typeof COLORS] ?? {
        line: '#6B7280',
    ...
    return {
      label: s.label,
      data: s.points.map((p: StoreGraphPoint) => p.v),
...
const StoreRevenueChart = ({ series, meta, isLoading }: IStoreRevenueChart) => {
  const t = useTranslations('storeDeliveriesDashboard.revenueChart');
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="h-[400px]">
        {series && series.length > 0 && series.some((item) => item.points.length > 0) ? (
