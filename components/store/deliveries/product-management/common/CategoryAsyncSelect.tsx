...
import { useGetSubCategoryParentOptions } from '@/hooks/api/store/deliveries/product-management/sub-categories';
import {
  resolveFormikError,
  type ErrorStrategy,
...
import CreateCategorySheet from '@/components/store/deliveries/product-management/categories/CreateCategorySheet';
...
export default function CategoryAsyncSelect({
  ...
  const tForm = useTranslations('subCategories.form');
  ...
  const { data, isLoading, isFetching, isError, refetch } =
    useGetSubCategoryParentOptions(queryParams, {
  ...
  const handleCreatedCategory = (createdCategory: Category) => {
    setAllCategories((current) =>
      mergeCategories(current, [createdCategory], initialSelectedCategory),
    );

    if (hasFormik) {
      formik!.setFieldValue(name, createdCategory.id);
    }
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            ...
            >
              {selectedCategory?.categoryName || placeholder}
            </span>
