...
import { useGetDealDropdownProducts } from '@/hooks/api/store/deliveries/product-management/deals';
import {
  resolveFormikError,
  type ErrorStrategy,
...
export default function ProductAsyncSelect({
  ...
  const renderProductText = (product: DealDropdownProduct) => {
    ...
    return `${product.name} (${formatCurrency(numericPrice, currencySymbol)})`;
  };
