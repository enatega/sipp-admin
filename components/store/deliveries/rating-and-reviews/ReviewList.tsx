...
import { useManageStoreReview } from '@/hooks/api/store/deliveries/rating-and-reviews';
import { handleApiError } from '@/lib/toast-error';
import {
  ApiErrorResponse,
  StoreManageReviewAction,
  StoreReviewItem,
} from '@/types';
...
const ReviewList = ({
  ...
  const tHideDialog = useTranslations('vendorRatingReviews.detail.list.hideDialog');
  const tDeleteDialog = useTranslations('vendorRatingReviews.detail.list.deleteDialog');
        ...
        {isLoading ? (
          ...
          <div className="py-6">
            <DisplayError
              ...
              message={errorMessage || tErrors('fetchFailedTitle')}
              variant="error"
            />
          </div>
        ) : reviews.length === 0 ? (
          ...
          reviews.map((item, index) => (
            <ReviewItem
              ...
              onAction={(reviewId, action) => setPendingAction({ reviewId, action })}
              disabled={isManaging}
            />
          ))
        )}
