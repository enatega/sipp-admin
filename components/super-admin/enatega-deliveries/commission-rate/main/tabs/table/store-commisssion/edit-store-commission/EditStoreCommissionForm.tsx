...
import { editStoreCommissionSchema } from '@/schemas/commission-rate/edit-store-commission.schema';
...
interface EditStoreCommissionFormProps {
  row?: StoreCommissionData | null;
  onClose: () => void;
}

export const EditStoreCommissionForm = ({
  row,
  onClose,
}: EditStoreCommissionFormProps) => {
  const t = useTranslations('commission-rate.storeForm');
  const tSchema = useTranslations('Schemas.commissionRate');
  const tCommon = useTranslations('common');
  const isEdit = Boolean(row);

  const { mutateAsync: updateStoreCommissionRate, isPending } =
  ...
  return (
    <Formik
      ...
      validationSchema={editStoreCommissionSchema(tSchema)}
      ...
      {({ values, setFieldValue, isSubmitting, dirty }) => (
        <Form className="flex flex-col gap-6 mt-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <Label htmlFor="status-mode" className="font-normal cursor-pointer">
