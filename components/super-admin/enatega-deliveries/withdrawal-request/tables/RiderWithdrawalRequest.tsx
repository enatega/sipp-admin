...
export default function RiderWithdrawalTable() {
  const tTable = useTranslations('withdrawalRequests.table');
  const [selectedRequest, setSelectedRequest] =
    useState<RiderWithdrawalRequest | null>(null);
  ...
  const riderWithdrawalRequests = (data?.data ||
    []) as RiderWithdrawalRequest[];
  ...
  const {
    ...
    requestSort,
    sortConfig,
  } = useSortableData(riderWithdrawalRequests);

  const handleAction = (
    request: RiderWithdrawalRequest,
    dialog: DialogType,
  ) => {
      ...
      <Table className="min-w-[1200px] w-full ">
        <TableHeader className="bg-accent">
          <TableRow className="!h-[55px]">
            ...
            />
            <TableHeaderCell
              label={tTable('riderBalance')}
              sortKey={'rider_balance'}
              requestSort={requestSort}
              sortConfig={sortConfig}
            />
            <TableHeaderCell
                ...
                <span className="flex items-center gap-2">
                  <Avatar className="h-10 w-10">
                    <AvatarFallback className="bg-accent">
                      {withdrawalRequest?.rider_name
                        ?.slice(0, 1)
                        ?.toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  {withdrawalRequest?.rider_name || notAvailable}
                </span>
              </TableCell>
