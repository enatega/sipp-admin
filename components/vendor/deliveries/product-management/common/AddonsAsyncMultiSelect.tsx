...
type AddonsAsyncMultiSelectProps = {
  ...
  initialSelectedAddons?: Array<Addon | VendorAddon> | null;
};

type AddonOption = Addon | VendorAddon;

const mergeAddons = (
  existing: AddonOption[],
  incoming: AddonOption[],
  initialSelectedAddons?: Array<Addon | VendorAddon> | null,
) => {
  const merged = new Map<string, AddonOption>();
...
export default function AddonsAsyncMultiSelect({
  ...
  const [allAddons, setAllAddons] = React.useState<AddonOption[]>(
    initialSelectedAddons ?? [],
  );
