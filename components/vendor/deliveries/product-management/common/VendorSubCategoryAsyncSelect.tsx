...
import { useGetProductSubCategoryOptions } from '@/hooks/api/vendor/deliveries/product-management/products';
import {
  resolveFormikError,
  type ErrorStrategy,
...
import CreateSubCategorySheet from '@/components/vendor/deliveries/product-management/sub-categories/CreateSubCategorySheet';

const PAGE_SIZE = 100;

type VendorSubCategoryAsyncSelectProps = {
  name: string;
  vendorId?: string;
  ...
  initialSelectedSubCategory?: VendorDropdownOption | null;
};
...
export default function VendorSubCategoryAsyncSelect({
  name,
  vendorId,
  parentCategoryId,
  label,
...
}: VendorSubCategoryAsyncSelectProps) {
  ...
  }, [formik, hasFormik, initialSelectedSubCategory, name, parentCategoryId, selectedSubCategoryId]);
  ...
  const { data, isLoading, isFetching, isError, refetch } =
    useGetProductSubCategoryOptions(queryParams, {
      enabled:
        (open || !!selectedSubCategoryId) && !!vendorId && !!parentCategoryId,
    });

  React.useEffect(() => {
    if (!data) return;

    const mappedSubCategories = data.data.map((item) => ({
      ...item,
      name: item.categoryName,
    })) as VendorDropdownOption[];

    setAllSubCategories((current) =>
      offset === 0
        ? mergeSubCategories([], mappedSubCategories, initialSelectedSubCategory)
        : mergeSubCategories(
            current,
            mappedSubCategories,
            initialSelectedSubCategory,
  ...
  const handleSelect = (subCategory: VendorDropdownOption) => {
    if (!hasFormik) return;
  ...
  const handleCreatedSubCategory = (createdSubCategory: {
    id: string;
    categoryName?: string;
    name?: string;
  }) => {
    const createdOption: VendorDropdownOption = {
      ...
      name: createdSubCategory.categoryName || createdSubCategory.name || '',
    };

    setAllSubCategories((current) =>
      mergeSubCategories(current, [createdOption], initialSelectedSubCategory),
    );
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            id={inputId}
            type="button"
            disabled={disabled || !vendorId || !parentCategoryId}
            className={cn(
              ...
              error
                ...
                : open && !disabled
                  ? 'border-primary'
                  : 'border-stroke',
              (disabled || !vendorId || !parentCategoryId) &&
          ...
          >
            ...
            >
              {selectedSubCategory?.name || placeholder || tForm('subcategoryPlaceholder')}
            </span>
            ...
            <div className="border-b p-3">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  aria-label={tForm('subcategoryRefresh')}
                >
                  <RefreshCw
