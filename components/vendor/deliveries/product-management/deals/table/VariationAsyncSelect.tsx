...
import { useGetDealDropdownVariations } from '@/hooks/api/vendor/deliveries/product-management/deals';
...
export default function VariationAsyncSelect({
  ...
  const queryParams = vendorId && productId
    ? {
        vendorId,
        productId,
        offset,
  ...
  const { data, isLoading, isFetching, isError, refetch } =
    useGetDealDropdownVariations(queryParams, {
      enabled: open && !!vendorId && !!productId,
    });
  ...
  const renderVariationText = (variation: DealDropdownVariation) => {
    ...
    return `${variation.name} (${formatCurrency(numericPrice, resolvedCurrencySymbol)})`;
  };
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            id={inputId}
            type="button"
            disabled={disabled || !vendorId || !productId}
            className={cn(
              ...
              <div className="flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <CommandPrimitive.Input
                    ...
                    placeholder={tForm('variationSearchPlaceholder')}
