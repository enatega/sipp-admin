  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <DownloadButtons<StoreOption>
        ...
        fetchAll={async () => (await fetchAllReport<StoreOption>('/apps/deliveries/chain-options/options', { params: { vendorId } })).map((option) => ({ ...option, title: (option as StoreOption & { name?: string }).name || option.title }))}
        className="mb-0"
      />
    </div>
  );
