...
export function PlanSelectionCard({ plan, billingCycle, isSelected, onSelect }: Props) {
  ...
  return (
    <button
      ...
      className={`relative w-full rounded-xl border bg-white p-4 text-left transition-colors ${
        isSelected ? 'border-primary ring-1 ring-primary/30' : 'border-stroke'
      } ${plan.isRecommended ? 'border-primary/60' : ''}`}
    >
      {plan.isRecommended ? (
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-mute">{plan.planDescription || 'No description available'}</p>
        </div>
        {plan.isActive ? (
            ...
            <div className="space-y-1.5">
              {items.map((title) => (
                <div key={title} className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-darkblack">{title}</span>
                </div>
              ))}
            </div>
          </div>
