import {
  ApiErrorResponse,
  GetEarningDashboardParams,
  GetEarningDashboardResponse,
  GetEarningViewParams,
  GetEarningViewResponse,
} from '@/types';
...
export const useGetEarningReport = (
  params?: GetEarningDashboardParams,
  options?: Omit<
    UseQueryOptions<GetEarningDashboardResponse, ApiErrorResponse>,
    'queryKey' | 'queryFn'
  >,
) => {
      ...
      : rawPeriod === 'weekly'
        ...
        : rawPeriod === 'monthly'
          ...
          : rawPeriod === 'yearly'
            ? 'all'
            : rawPeriod;
  const allowedPeriods: NonNullable<GetEarningDashboardParams['period']>[] = [
    'today',
    'this_week',
    'this_month',
    'all',
  ...
  const period = allowedPeriods.includes(
    ...
    : undefined;
  const startDate =
    period === 'custom' ? getParam('startDate') || undefined : undefined;
  const endDate =
    period === 'custom' ? getParam('endDate') || undefined : undefined;
  ...
  return useQuery<GetEarningDashboardResponse, ApiErrorResponse>({
    ...
    queryFn: async () => {
      const response = await Axios.get<GetEarningDashboardResponse>(
        '/apps/deliveries/admin/earning-reports',
        {
...
export const useGetEarningView = (
  options?: Omit<
    UseQueryOptions<GetEarningViewResponse, ApiErrorResponse>,
    'queryKey' | 'queryFn'
  >,
) => {
  const { getParam } = useQueryParams();
  ...
  const startDate = rawStartDate
    ? new Date(rawStartDate).toISOString()
    : undefined;
  ...
  return useQuery<GetEarningViewResponse, ApiErrorResponse>({
    ...
    queryFn: async () => {
      ...
      if ((params as typeof params & { modeScope?: string }).modeScope)
        query.append(
          'modeScope',
          (params as typeof params & { modeScope?: string })
            .modeScope as string,
        );
